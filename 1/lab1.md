...
Для настройки сетевого взаимодействия с СУБД был отредактирован конфигурационный файл /etc/postgresql/15/main/postgresql.conf. В секции «Connections and Authentication» параметр port был изменен со значения по умолчанию 5432  на значение 5433. Данное изменение позволяет запустить сервер PostgreSQL на нестандартном порту, что может требоваться для обеспечения безопасности или во избежание конфликтов с другими экземплярами баз данных.
